...
public class DriveCommands {
  ...
  /*
   * Auto Target to a April Tag but still be able to drive
   */

  public static Command autoTarget(
      Drive drive,
      DoubleSupplier x,
      DoubleSupplier y,
      Supplier<Rotation2d> visionSupplier,
      CommandXboxController noVisController) {

    ProfiledPIDController angularPID =
    ...
    return Commands.run(
            () -> {
              ...
              Translation2d linearVelocity =
                  getLinearVelocityFromJoysticks(x.getAsDouble(), y.getAsDouble());

              // Robot heading
              double heading = drive.getRotation().getRadians();

              // Vision Offset
              double visionOffset = visionSupplier.get().getRadians();

              // Caluclation PID for omega
              double omega = angularPID.calculate(heading, heading - visionOffset);
            ...
            drive)
        .beforeStarting(() -> angularPID.reset(drive.getRotation().getRadians()));
  }
